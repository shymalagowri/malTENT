...
def topk(x,
    ...
    for n in range(k):
        ...
        # print("Printing loss for xnvar .....",loss)

        # compute gradient
        grads = torch.autograd.grad(loss.mean(), xn_var)[0].data
        
        # print("Printing grad .....",grads)
        ...
        # print("Printing xnext (after getting selected topk indices)",x_next)
        # projection
        x_next = clip_tensor(x_next)
        # print("Printing clamped tensor...",x_next)
        x_next = or_float_tensors(x_next, x)
        # print("OR function on x_next and x",x_next)
        
        # compute adversarial loss
        loss_adv = loss_fct(model(Variable(x_next)), y)
        # print("Adversarial loss...",loss_adv)
...
def topkrestart(x,
    ...
    y_model = model(Variable(x))
    loss_natural = loss_fct(y_model, y).data
    # print("Printing loss ...",loss_natural)
    x_next = x.clone()
    ...
    for i in range(10):
        ...
        print("Printing x_rand",x_rand)
        x_next = topk(x,y.data,model,loss_fct,k,epsilon,alpha,False,use_sample,x_rand)
        print("Printing x_next based on topk",x_next)
        y_model = model(Variable(x_next.cuda()))
        loss_adv = loss_fct(y_model, y)
        print("Printing adv loss",loss_adv)
        print("Printing best loss variable",best_loss)
    ...
    x_next = x_best.clone()
    x_next = clip_tensor(x_next)
    print("Printing x_next after clipping",x_next)
...
def dfgsm_k(x,
    ...
    for t in range(k):
        ...
        y_model = model(x_var)
        loss = loss_fct(y_model, y)
        print("Printing loss for xnext",loss)

        losses.append(loss.cpu().data.numpy()[0])
        # compute gradient
        grad_vars = torch.autograd.grad(loss.mean(), x_var)
        print("Printing grad.......",grad_vars)
        

        # find the next sample
        # print("Printing grad in inner maximizer........ : ",grad_vars[0].data)
        print("Printing sign of gradient..... ",torch.sign(grad_vars[0].data))
        x_next = x_next + epsilon * torch.sign(grad_vars[0].data)
        print("Printing x_next",x_next)
        # projection
        x_next = clip_tensor(x_next)
        print("Clipped tensor....",x_next)

    # rounding step
    x_next = round_x(x_next, alpha=alpha)

    print("Printing round off.....",x_next)

    # feasible projection
    x_next = or_float_tensors(x_next, x)
    print("OR operation on xnext and x....",x_next)
    ...
    print("Printing loss_adv......",loss_adv)
    print("Printing loss_natural.........",loss_natural)
...
def rfgsm_k(x, y, model, loss_fct, k=25, epsilon=0.02, is_report_loss_diff=False, use_sample=False, return_loss=False):
    ...
    for t in range(k):
        ...
        loss = loss_fct(y_model, y)
        print("Printing loss for xvar ...",loss)
...
def grosse_k(x, y, model, loss_fct, k=25, is_report_loss_diff=False, use_sample=False, return_loss=False):
    ...
    for t in range(k):
        ...
        losses.append(loss.cpu().data.numpy()[0])
        print("Adversarial loss in inner maximizer",loss)
        print("loss_worst ...",loss_worst)
